...
const db = SQLite.openDatabaseSync('simplicity.db');
...
export const dbOperations = {
  ...
  createTask: (task: TaskInsert, deadline: string): Task => {
    ...
    const newTask = db.getFirstSync('SELECT * FROM tasks WHERE id = ?', [result.lastInsertRowId]);
    return newTask as Task;
  },

  // Update task
  updateTask: (taskId: number, updates: { title?: string; description?: string; deadline?: string }): void => {
    const task = db.getFirstSync('SELECT * FROM tasks WHERE id = ?', [taskId]) as Task | null;
  ...
  reEnableTask: (originalTaskId: number): Task => {
    const originalTask = db.getFirstSync('SELECT * FROM tasks WHERE id = ?', [originalTaskId]) as Task | null;
  ...
  getStats: (startDate: string, endDate: string) => {
    ...
    const unfinished = db.getFirstSync(
      'SELECT COUNT(*) as count FROM tasks WHERE status = ? AND created_at BETWEEN ? AND ?',
      ['expired_unfinished', startDate, endDate]
    ) as any;

    const totalAttempts = db.getFirstSync(
